...
const mockEnquiries = [
  {
    ...
    firstName: "James",
    lastName: "Holden",
    email: "j.holden@thecraftybar.co.uk",
    whoAreYou: "Bar/Restaurant Owner",
    message: "Hi team, we are opening a new cocktail bar in Manchester next month and would love to stock your Lemon Italian Spritz on draft or in bulk. Do you have a wholesale price list?",
    date: "Mar 21, 2026",
    status: "New"
  },
  {
    ...
    whoAreYou: "Consumer",
    message: "Hello! I bought a 4-pack of the Orange Spritz at the Foodies Festival last week. Absolutely loved it. When will it be back in stock online?",
    date: "Mar 20, 2026",
    status: "Replied"
  },
  {
    ...
    firstName: "Marcus",
    lastName: "Aurelius",
    email: "marcus@rome-distributors.eu",
    whoAreYou: "Distributor",
    message: "We distribute premium non-alcoholic beverages across mainland Europe. Interested in discussing an exclusive partnership for Italy and Spain.",
    date: "Mar 18, 2026",
    status: "Read"
  },
  {
    ...
    firstName: "Sophie",
    lastName: "Turner",
    email: "sophie.t88@hotmail.com",
    whoAreYou: "Other",
    message: "Hi, I'm organizing a corporate summer party for 200 people. Do you cater for events like this directly?",
    date: "Mar 15, 2026",
    status: "Replied"
  }
];
...
function ContactEnquiries() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'New':
        return <span className="bg-amber-50 text-amber-700 border border-amber-200/50 px-2.5 py-1 rounded-md text-xs font-semibold flex items-center gap-1 w-max"><Clock size={12} /> New</span>;
      case 'Read':
        return <span className="bg-sky-50 text-sky-700 border border-sky-200/50 px-2.5 py-1 rounded-md text-xs font-semibold flex items-center gap-1 w-max"><Eye size={12} /> Read</span>;
      case 'Replied':
        return <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/50 px-2.5 py-1 rounded-md text-xs font-semibold flex items-center gap-1 w-max"><CheckCircle size={12} /> Replied</span>;
      default:
  ...
  // Handle opening the drawer and marking as read
  const handleOpenDrawer = (enquiry: any) => {
    setSelectedEnquiry(enquiry);
    setReplyMessage(""); // Clear previous draft
    
    // In a real app, you would fire an API call here to change status from 'New' to 'Read'
  };
  ...
  const handleSendReply = () => {
    ...
    setIsSending(true);
    // Simulate API call to send email via PHP
    setTimeout(() => {
      setIsSending(false);
                    ...
                    <tr key={enq.id} onClick={() => handleOpenDrawer(enq)} className={`hover:bg-slate-50/50 transition-colors group cursor-pointer ${enq.status === 'New' ? 'bg-amber-50/10' : ''}`}>
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className={`text-sm ${enq.status === 'New' ? 'font-bold text-slate-900' : 'font-medium text-slate-700'}`}>
                      ...
                      <td className="p-4 hidden md:table-cell max-w-xs">
                        <p className={`text-sm truncate ${enq.status === 'New' ? 'font-semibold text-slate-800' : 'text-slate-500'}`}>
                          {enq.message}
                        </p>
                      </td>
                ...
                <div className="p-4 text-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-slate-600">
                      ...
                      <a href={`mailto:${selectedEnquiry.email}`} className="font-medium hover:text-indigo-600 hover:underline">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex justify-end">
                    <button 
                      ...
                      className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Send size={16} />
