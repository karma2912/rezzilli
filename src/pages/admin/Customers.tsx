...
// --- DUMMY DATA (Enriched with order items and financials for the drill-down view) ---
const mockCustomers = [
  {
    id: "CUST-001",
    name: "Sarah Jenkins",
    ...
    recent_orders: [
      { 
        ...
        items: [{ name: "Lemon Italian Spritz", variant: "24 Bottles", quantity: 2, price: 24.00, image: "/image4.png" }],
        financials: { subtotal: 48.00, shipping: 0.00, discount: 0.00, total: 48.00 },
        gift_message: "Happy Birthday Mum! Love, Sarah x", payment_method: "Credit Card"
      },
      { 
        id: "ORD-0742", date: "Feb 02, 2026", total: "£62.00", status: "Delivered",
        items: [{ name: "Summer Berry Spritz", variant: "12 Bottles", quantity: 2, price: 18.00, image: "/image4.png" }, { name: "Limited Edition Grapefruit", variant: "24 Bottles", quantity: 1, price: 26.00, image: "/image6.png" }],
        financials: { subtotal: 62.00, shipping: 0.00, discount: 0.00, total: 62.00 },
        gift_message: null, payment_method: "PayPal"
      },
      { 
        id: "ORD-0610", date: "Jan 15, 2026", total: "£24.00", status: "Delivered",
        items: [{ name: "Lemon Italian Spritz", variant: "24 Bottles", quantity: 1, price: 24.00, image: "/image4.png" }],
        financials: { subtotal: 24.00, shipping: 5.00, discount: 5.00, total: 24.00 },
        gift_message: null, payment_method: "Credit Card"
      }
    ]
  },
  {
    ...
    recent_orders: [
      { 
        ...
        items: [{ name: "Non-Alcohol Orange Spritz", variant: "4 Bottles", quantity: 3, price: 36.79, image: "/image6.png" }],
        financials: { subtotal: 110.37, shipping: 0.00, discount: 0.00, total: 110.37 },
        gift_message: null, payment_method: "Credit Card"
      }
    ]
  }
...
function Customers() {
  ...
  const [selectedOrderDetails, setSelectedOrderDetails] = useState<any>(null); // NEW: Tracks which specific order is being viewed
  ...
  const getOrderStatusBadge = (status: string) => {
    switch (status) {
      case 'Processing': return <span className="bg-amber-50 text-amber-700 border border-amber-200/50 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider">{status}</span>;
      case 'Shipped': return <span className="bg-sky-50 text-sky-700 border border-sky-200/50 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider">{status}</span>;
      case 'Delivered': return <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/50 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider">{status}</span>;
      default: return <span className="bg-slate-50 text-slate-700 border border-slate-200/50 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider">{status}</span>;
    }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
                    ...
                    <tr key={customer.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-sm shrink-0 border border-indigo-100">{getInitials(customer.name)}</div>
                          <div>
                      ...
                      <td className="p-4 text-right">
                        <button onClick={() => handleOpenDrawer(customer)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-sm">
      ...
      {selectedCustomer && (
        <>
          <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-40 transition-opacity" onClick={handleCloseDrawer} />
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-[500px] bg-white z-50 shadow-2xl flex flex-col transform transition-transform duration-300">
            ...
            <div className="px-6 py-6 border-b border-slate-100 bg-white shrink-0 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xl shrink-0 border border-indigo-100">{getInitials(selectedCustomer.name)}</div>
                  <div>
                ...
                <button onClick={handleCloseDrawer} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-md transition-colors"><X size={20} /></button>
              </div>
            </div>
            ...
            {selectedOrderDetails ? (
              ...
              <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 space-y-4">
                <button onClick={() => setSelectedOrderDetails(null)} className="flex items-center gap-1 text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors mb-2">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-bold text-slate-900">{selectedOrderDetails.id}</p>
                  </div>
                ...
                {selectedOrderDetails.gift_message && (
                  <div className="bg-rose-50 border border-rose-100 p-5 rounded-xl flex items-start gap-4">
                    <div className="p-2 bg-rose-100 text-rose-600 rounded-lg shrink-0"><Gift size={20} /></div>
                    <div>
                      <h4 className="text-sm font-bold text-rose-900 mb-1">Gift Message Included</h4>
                      <p className="text-sm text-rose-800 italic leading-relaxed">"{selectedOrderDetails.gift_message}"</p>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-slate-900">Items ({selectedOrderDetails.items.length})</h3>
                  </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
                  ...
                  <div className="p-5 text-sm space-y-3">
                    <div className="flex justify-between text-slate-600">
                      <span>Subtotal</span><span>£{selectedOrderDetails.financials.subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-slate-600">
                      <span>Shipping</span><span>{selectedOrderDetails.financials.shipping === 0 ? 'Free' : `£${selectedOrderDetails.financials.shipping.toFixed(2)}`}</span>
                    </div>
                    {selectedOrderDetails.financials.discount > 0 && (
                      <div className="flex justify-between text-emerald-600">
                        <span>Discount</span><span>-£{selectedOrderDetails.financials.discount.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-3 border-t border-slate-100 mt-2">
                      <span className="font-bold text-slate-900">Total</span>
                      <span className="font-bold text-lg text-indigo-600">£{selectedOrderDetails.financials.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 space-y-4">
                <button onClick={() => setIsViewingAllOrders(false)} className="flex items-center gap-1 text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors mb-2">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-bold text-slate-900">Complete Order History</h3>
                    </div>
                    <span className="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md text-xs font-semibold">{selectedCustomer.total_orders} Orders</span>
                  </div>
                  
                  <div className="divide-y divide-slate-100">
                    {selectedCustomer.recent_orders.map((order: any, idx: number) => (
                      <div key={idx} onClick={() => setSelectedOrderDetails(order)} className="p-5 hover:bg-slate-50 transition-colors flex items-center justify-between group cursor-pointer">
                        <div>
                          <p className="text-sm font-bold text-indigo-600 group-hover:text-indigo-700 transition-colors">{order.id}</p>
              ...
              <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 space-y-6">
                  ...
                  <div className="p-5 text-sm space-y-4">
                    <div className="grid grid-cols-1 gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <a href={`mailto:${selectedCustomer.email}`} className="text-indigo-600 hover:underline font-medium">{selectedCustomer.email}</a>
                      </div>
                      <div className="flex items-center gap-3">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
                  <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <button onClick={() => setIsViewingAllOrders(true)} className="text-xs font-semibold text-indigo-600 hover:text-indigo-700">
                      View All
                    </button>
                  </div>
                  ...
                  <div className="divide-y divide-slate-100">
                    {selectedCustomer.recent_orders.length === 0 ? (
                      ...
                      selectedCustomer.recent_orders.map((order: any, idx: number) => (
                        <div key={idx} onClick={() => setSelectedOrderDetails(order)} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between group cursor-pointer">
                          <div>
