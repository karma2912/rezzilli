...
// --- DUMMY DATA (Structured exactly how your get-profile.php sends it) ---
const mockOrders = [
  {
    id: "ORD-0891",
    date: "March 18, 2026",
    ...
    gift_message: "Happy Birthday Mum! Hope you enjoy these in the garden. Love, Sarah x",
    shipping: {
    ...
    items: [
      { name: "Lemon Italian Spritz", variant: "24 Bottles", quantity: 1, price: 24.00, image: "/image4.png" },
      { name: "Non-Alcohol Orange Spritz", variant: "24 Bottles", quantity: 1, price: 24.00, image: "/image6.png" }
    ],
  ...
  },
  {
    ...
    items: [
      { name: "Non-Alcohol Orange Spritz", variant: "4 Bottles", quantity: 3, price: 36.79, image: "/image6.png" }
    ],
  ...
  },
  {
    ...
    financials: { subtotal: 24.00, shipping: 5.00, discount: 0.00, total: 29.00 }
  }
];
...
function Orders() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Processing':
        return <span className="bg-amber-50 text-amber-700 border border-amber-200/50 px-2.5 py-1 rounded-md text-xs font-semibold">Processing</span>;
      case 'Shipped':
        return <span className="bg-sky-50 text-sky-700 border border-sky-200/50 px-2.5 py-1 rounded-md text-xs font-semibold">Shipped</span>;
      case 'Delivered':
        return <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/50 px-2.5 py-1 rounded-md text-xs font-semibold">Delivered</span>;
      default:
    ...
    <div className="w-full min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 sticky top-0 z-10">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage fulfillment, view details, and track shipping.</p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-500">No orders found matching your criteria.</td>
                  </tr>
                ) : (
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-4 font-semibold text-sm text-indigo-600">{order.id}</td>
                      <td className="p-4 text-sm text-slate-600">{order.date}</td>
                      <td className="p-4">
                        <p className="text-sm font-medium text-slate-900">{order.customer}</p>
                        <p className="text-xs text-slate-500">{order.shipping.city}, {order.shipping.country}</p>
                      </td>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-[500px] bg-white z-50 shadow-2xl flex flex-col transform transition-transform duration-300">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">{selectedOrder.date}</p>
              </div>
              <button 
                onClick={() => setSelectedOrder(null)}
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 space-y-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                ...
                <div className="flex flex-col items-end">
                  ...
                  <select className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer">
                    <option value="Processing">Processing</option>
                    <option value="Shipped">Shipped</option>
                    <option value="Delivered">Delivered</option>
                  </select>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-900">Customer & Shipping</h3>
                </div>
                <div className="p-5 text-sm">
                  <p className="font-semibold text-slate-900 mb-1">{selectedOrder.customer}</p>
                  <p className="text-slate-600 mb-4 hover:text-indigo-600 cursor-pointer">{selectedOrder.email}</p>
                  
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                    <p className="text-slate-700 font-medium mb-1">{selectedOrder.shipping.name}</p>
                    <p className="text-slate-600 leading-relaxed">
                      {selectedOrder.shipping.line1}<br/>
                      {selectedOrder.shipping.city}, {selectedOrder.shipping.region} {selectedOrder.shipping.zip}<br/>
                      {selectedOrder.shipping.country}
                    </p>
                    <p className="text-slate-500 mt-2 text-xs font-medium">Phone: {selectedOrder.shipping.phone}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-900">Items ({selectedOrder.items.length})</h3>
                </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
                ...
                <div className="p-5 text-sm space-y-3">
                  ...
                  <div className="flex justify-between text-slate-600">
                    <span>Shipping</span>
                    <span>{selectedOrder.financials.shipping === 0 ? 'Free' : `£${selectedOrder.financials.shipping.toFixed(2)}`}</span>
                  </div>
                  {selectedOrder.financials.discount > 0 && (
                    <div className="flex justify-between text-emerald-600">
                      <span>Discount</span>
                      <span>-£{selectedOrder.financials.discount.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center pt-3 border-t border-slate-100 mt-2">
                    <span className="font-bold text-slate-900">Total</span>
                    <span className="font-bold text-lg text-indigo-600">£{selectedOrder.financials.total.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center pt-3 mt-1">
                    <span className="text-xs text-slate-500 uppercase tracking-wider font-bold">Method</span>
                    <span className="text-xs font-semibold text-slate-700 bg-slate-100 px-2 py-1 rounded">{selectedOrder.payment_method}</span>
                  </div>
                </div>
              </div>
